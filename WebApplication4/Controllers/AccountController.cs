...
namespace WebApplication4.Controllers
{
    ...
    public class AccountController : IdentityControllerBase
    {
        ...
        {
            var result = await AccountManager.FacebookLogin(model.AccessToken, new UserModel(model.FullName, model.BirthDay, model.City, model.WorkPlace));
            if (result.Success)
            {
                UserToken = result.Token;
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            var result = await AccountManager.RegisterAsync(model.Username,model.Password,new UserModel( model.FullName,model.BirthDate,model.City,model.WorkPlace));
            if (result.Success)
            {
                UserToken = result.Token;
                return RedirectToAction("Index", "Home");
